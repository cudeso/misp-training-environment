...
mysqlImport () {
  ...
  if [[ "$myDB" == "$MISPDB" ]]; then
    TmpUsersSql="$(mktemp --tmpdir=$OutputDirName)"
    echo -n "$myDB detected"
    REMOVE="y"
    # Export Users table
    mysqldump --opt --host $MISPDBHost -u $MySQLRUser -p$MySQLRPassword $myDB users > $TmpUsersSql 
    [[ "$REMOVE" == "y" ]] && mysql -h $MISPDBHost -u $MySQLRUser -p$MySQLRPassword -e "DROP DATABASE IF EXISTS $myDB"

    mysql -h $MISPDBHost -u $MySQLRUser -p$MySQLRPassword -Bse "CREATE DATABASE IF NOT EXISTS $MISPDB ;\
    ...
    mysql -s -h $MISPDBHost -u $MySQLRUser -p$MySQLRPassword -D $MISPDB < $TmpUsersSql 
    rm $TmpUsersSql
  fi
}
